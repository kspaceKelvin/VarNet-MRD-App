...
# Wrap in a try so that varnet (and its dependency PyTorch) aren't a mandatory requirement
try:
    import varnet
except:
    pass
class Server:
    ...
    def handle(self, sock):
        ...
        try:
            ...
            elif ("varnet" in config):
                logging.info("Starting varnet processing based on config")
                varnet.process(connection, config, metadata)
            elif (config == "null"):
